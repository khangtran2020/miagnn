...
def shadow_visualization(graph:dgl.DGLGraph, path:str, name:str):
    ...
    if os.path.exists(path=path):
        ...
        # target graph
        pos_postr_x = np.random.normal(loc=-2.0, scale=0.5, size=len(id_postr))
        pos_postr_y = np.random.normal(loc=2.0, scale=0.5, size=len(id_postr))
        pos_postr = list(zip(pos_postr_x, pos_postr_y))
        pos = dict(zip(id_postr, pos_postr))

        pos_negtr_x = np.random.normal(loc=-2.0, scale=0.5, size=len(id_negtr))
        pos_negtr_y = np.random.normal(loc=-2.0, scale=0.5, size=len(id_negtr))
        pos_negtr = list(zip(pos_negtr_x, pos_negtr_y))
        pos.update(dict(zip(id_negtr, pos_negtr)))

        pos_poste_x = np.random.normal(loc=2.0, scale=0.5, size=len(id_poste))
        pos_poste_y = np.random.normal(loc=2.0, scale=0.5, size=len(id_poste))
        pos_poste = list(zip(pos_poste_x, pos_poste_y))
        pos.update(dict(zip(id_poste, pos_poste)))

        pos_negte_x = np.random.normal(loc=2.0, scale=0.5, size=len(id_negte))
        pos_negte_y = np.random.normal(loc=-2.0, scale=0.5, size=len(id_negte))
        pos_negte = list(zip(pos_negte_x, pos_negte_y))
        pos.update(dict(zip(id_negte, pos_negte)))
